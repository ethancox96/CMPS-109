...
cout << "in ellipse constructor" << endl;
   DEBUGF ('c', this);
}
...
equilateral::equilateral (const vertex_list& v): triangle(v) {
   DEBUGF ('c', this);
}
...
diamond::diamond (GLfloat width, GLfloat height): polygon({{0.0, 0.0}, 
           {width/2, -height/2}, {width, 0.0}, {width/2, height/2}}) {
    DEBUGF ('c', this << "(" << width << "," << height << ")");
}
...
void polygon::draw (const vertex& center, const rgbcolor& color) const {
   ...
   glColor3ubv(color.ubvec);
   for(auto p: vertices) {
      glVertex2f (p.xpos+center.xpos, p.ypos+center.ypos);
   }
   glEnd();
}
